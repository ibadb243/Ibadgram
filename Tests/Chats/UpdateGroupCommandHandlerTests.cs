...
namespace Tests.Chats
{
    public class UpdateGroupCommandHandlerTests
    {
        ...
        private Mock<ILogger<UpdateGroupCommandHandler>> _loggerMock;
        private UpdateGroupCommandHandler _handler;
        ...
        public UpdateGroupCommandHandlerTests()
        {
            ...
            _loggerMock = new Mock<ILogger<UpdateGroupCommandHandler>>();
            ...
            _handler = new UpdateGroupCommandHandler(
                _unitOfWorkMock.Object,
                _loggerMock.Object);
        }

        [Fact]
        public async Task Handle_ValidCommand_UpdatesGroupSuccessfully()
        {
            // Arrange
            ...
            _chatRepositoryMock.Verify(r => r.UpdateAsync(It.Is<Chat>(c =>
                c.Id == groupId &&
                c.Name == command.Name &&
        ...
        public async Task Handle_PartialUpdate_UpdatesOnlySpecifiedFields()
        {
            ...
            _chatMemberRepositoryMock.Setup(r => r.GetByIdsAsync(groupId, userId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(member);
            _chatRepositoryMock.Setup(r => r.UpdateAsync(It.IsAny<Chat>(), It.IsAny<CancellationToken>()))
                .Callback<Chat, CancellationToken>((c, ct) => capturedChat = c)
            ...
            capturedChat.Name.Should().Be(command.Name);
            capturedChat.Description.Should().Be(group.Description);

            _chatRepositoryMock.Verify(r => r.UpdateAsync(It.IsAny<Chat>(), It.IsAny<CancellationToken>()), Times.Once);
            _unitOfWorkMock.Verify(u => u.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
            _unitOfWorkMock.Verify(u => u.CommitTransactionAsync(It.IsAny<CancellationToken>()), Times.Once);
        }
